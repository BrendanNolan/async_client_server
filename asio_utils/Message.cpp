...
utils::MessageHeader::MessageHeader(std::uint32_t type, std::uint32_t size) : type_{ type }, bodySize_{ size } {}
...
utils::Message &utils::operator<<(utils::Message &message, const std::string &data) {
  std::transform(
    data.begin(), data.end(), std::back_inserter(message.body_), [](char c) { return static_cast<std::uint8_t>(c); });
...
void utils::resizeBodyAccordingToHeader(Message &message) { message.body_.resize(message.header_.bodySize_); }
